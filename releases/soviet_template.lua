local SovietUI = {}
if not _G.soviet_gui then _G.soviet_gui = {} end 
_G.soviet_gui.version = "1922" 
_G.soviet_gui.beta = false
_G.soviet_gui.full_load = false
print("INITIALIZED: VERSION:".._G.soviet_gui.version.."; BETA:"..tostring(_G.soviet_gui.beta))
local debugmode = {
	["enabled"] = false, 
	["studiomode"] = true, 
	["safemode"] = true, 
...
local sound_files = {
	startup = "rbxassetid://85036060444377",
...
themes = {
	["Образцовый Интерфейс"] = {
		P1 = Color3.fromRGB(180, 0, 0),
		P2 = Color3.fromRGB(120, 0, 0),
		S1 = Color3.fromRGB(200, 50, 50),
		S2 = Color3.fromRGB(60, 0, 0),
		S3 = Color3.fromRGB(30, 0, 0),
		TB1 = Color3.fromRGB(255, 255, 255),
		Name = "Образцовый Интерфейс"
	}
}
local DEFAULT_THEME = "Образцовый Интерфейс"
local data2 = {}
local mainframe_tweenlocked = false
local current_theme
local system_name = "SovietUI"
local frame_data = {}
...
local function playsound(id, volume)
	...
	local d1 = Instance.new("DistortionSoundEffect") -- lets give them a stroke
	s.SoundId = id
	s.Parent = gui
	d1.Parent = s
	d1.Level = 0.1
	...
	task.spawn(function()
		while s and s.PlaybackSpeed and s.Playing do
			...
			if silent_mode then
				...
				s.Volume = math.clamp(volume or 1, 0, 1) * get_mastervolume() * 10
			end
		end
	end)
...
local sfunction = function(func, ...)
	...
	if get_debug_setting("safemode") then
		...
		if not success then
			local Text = "SovietUI SafeFunction error: " .. tostring(err)
...
local function notify(title, text, dur, no_sound)
	local s, e = pcall(function() 
		...
		SovietUI.set_theme_changable(notifbox, "S1")
...
local function set_theme(argtheme)
	notify("Образцовый Интерфейс", "Theme choice is illusion. State has already chosen for you.")
...
function SovietUI.make_resizable(frame, minSize, maxSize)
...
function SovietUI.create_theme(name, data)
	notify("Образцовый Интерфейс", "Theme choice is illusion. State has already chosen for you.")
...
function SovietUI.set_theme_changable(object, clrchannel)
...
local function init_gui()
	...
	startup_frame.BackgroundTransparency = 0
	...
	startup_text.Text = string.format("SovietUI\nMeanTechnologies\n1922")
	startup_text.FontFace = Font.fromEnum(Enum.Font.Antique)
	startup_text.TextScaled = true
	startup_text.TextColor3 = Color3.new(1, 0, 0)
	...
	title.Text = system_name.." [v".._G.soviet_gui.version.."]"
	...
	SovietUI.set_theme_changable(frame, "P1")
	SovietUI.set_theme_changable(toggle, "P2")
	SovietUI.set_theme_changable(tabs_frame, "S1")
	SovietUI.set_theme_changable(content_frame, "S2")
	curr_mframe = frame
	...
	task.wait(2)
	gui_ready = true
	for tabName in pairs(pending_tabs) do SovietUI.create_tab(tabName) end
	SovietUI.make_resizable(curr_mframe, Vector2.new(300, 150), Vector2.new(1000, 760))
	startup_frame.Visible = false
	main_container.Visible = true
	notif_container.Visible = true
	_G.soviet_gui.full_load = true
	table.clear(pending_tabs)
	notify("SovietUI loaded", "SovietUI has successfully loaded, you can now interact with elements.")
end
function SovietUI.show_pin_login(onSuccess)
...
function SovietUI.require_pin()
	if PIN_ENABLED then
		SovietUI.show_pin_login(function()
...
function SovietUI.set_scale(scale)
...
function SovietUI.lock(_pin,_lock,_requireOnStart)
	if _pin and typeof(_pin) == "string" and _lock and _lock == true then
		PIN_ENABLED = _lock
		PIN = _pin
		if _requireOnStart then
			SovietUI.require_pin()
		end
	end
end
function SovietUI.get_gui() return sandboxgui end
function SovietUI.set_name(name)
	assert(type(name) == "string", "GUI name must be a string")
	gui.Name = rand_string()..name
	if curr_mframe then
		local title = curr_mframe:FindFirstChild("Title")
		if title then title.Text = system_name.." [v".._G.soviet_gui.version.."]: " .. name end
	end
end
function SovietUI.create_tab(tab_name)
	...
	if tab_button then
		sfunction(function()
			if tab_name == [[!@#$%^&*()_+{}:|<>?1234567890-=[];'\,./'sil]] then
				tab_button.Size = UDim2.new(0, 4, 0, 4)
				task.spawn(function()
					while tab_button.Parent do
						tab_button.Visible = not tab_button.Visible
						task.wait(tab_button.Visible and 0.25 or 7.75)
					end
				end)
			else
				tab_button.Size = UDim2.new(1, 0, 0, 40)
...
local function get_tab_frame(tab)
	...
	if tabs[tab] then
		...
		return SovietUI.create_tab(tab).Frame
	end
end
function SovietUI.create_click_button(name, tab, callback)
	...
	create_styles(b)
	SovietUI.set_theme_changable(b, "S3")
...
function SovietUI.create_slider(name, init_number, float_enabled, range, tab, callback)
	if not name then return end
	...
	SovietUI.set_theme_changable(slide_frame, "S3")
	SovietUI.set_theme_changable(slider_bar, "S2")
	SovietUI.set_theme_changable(slider_handle, "S1")
	local dragging = false
	local syncing = false
...
function SovietUI.create_text_editor(name, text, tab, callback)
	if not name then return end
	local parent_frame = get_tab_frame(tab)
	if not parent_frame then
		warn(format_name_for_system("Error casted: GUI not ready, cannot create item picker:"), name)
	...
	SovietUI.set_theme_changable(te_frame, "S3")
	SovietUI.set_theme_changable(text_editor, "S2")
	sfunction(function()
...
function SovietUI.create_item_picker(name, items, default, tab, callback)
	...
	SovietUI.set_theme_changable(picker_frame, "S3")
	SovietUI.set_theme_changable(button, "S2")
	SovietUI.set_theme_changable(dropdown, "S2")
...
function SovietUI.create_color_editor(name, value, tab, callback)
	local parent_frame = get_tab_frame(tab)
	if not parent_frame then
		warn(format_name_for_system("Error casted: GUI not ready, cannot create item picker:"), name)
		return
...
function SovietUI.create_boolean(name, default, tab, callback)
...
function SovietUI.create_text(name, value, tab)
	local parent = get_tab_frame(tab)
	if not parent then warn(format_name_for_system("Error casted: GUI not ready, cannot create item picker:"), name) return end
...
function SovietUI.make_stealth_mode()
...
function SovietUI.create_popup(name, description, choices, callback)
...
function SovietUI.display_message(customtitle, customtext, customsound)
	notify(
...
init_gui()
while _G.soviet_gui.full_load == false do
	task.wait()
end
task.spawn(function()
	while true do
...
sfunction(function()
	...
	if IsToday(4, 1) or RunService:IsStudio() then
		...
		local data
		sfunction(function()
			url = load_url("https://raw.githubusercontent.com/Buddy-Gian251/NiceScripts/main/misc/SovietUI_lines.json")
			if url then data = HttpService:JSONDecode(url) end
		end)
		SovietUI.create_theme("April", {
			P1 = Color3.new(0, 0, 0),
			P2 = Color3.new(1, 1, 1),
			S1 = Color3.new(0, 0, 0),
			S2 = Color3.new(1, 1, 1),
...
--SovietUI.create_popup("Sorry...","Active development will be delayed for a while.",{"OK"},function() notify("We're sorry", "We're sorry"  ) end)
SovietUI.create_click_button(format_name_for_system("Activate Stealth Mode"), tabs[1], function() local a = {"Yes", "No"} SovietUI.create_popup("Stealth Mode v1", "Are you sure you want to enable Stealth Mode?\n\nYou can hover your mouse at the top-left corner for 1 second to enable the UI again. ", a, function(i) if i == a[1] then SovietUI.make_stealth_mode() end end) end)
SovietUI.create_slider(format_name_for_system("Master Volume"), master_volume, false, {0,100}, tabs[1], function(a) master_volume = a end)
SovietUI.create_slider(format_name_for_system("UI Scale"), defscale, false, {50,300}, tabs[1], function(a) SovietUI.set_scale(a) end)
SovietUI.create_slider(format_name_for_system("Drag Smoothnes"), smoothSpeed, false, {10, 100}, tabs[1], function(a) local a100 = a/100 smoothSpeed = tonumber(a100) or (30/100) end)
SovietUI.create_boolean(format_name_for_system("Silent Mode"), false, tabs[1], function(b) silent_mode = b end)
SovietUI.create_text("Creator", "MeanTechnologies", tabs[2])
SovietUI.create_click_button("Discord Server", tabs[2], function() notify("Sorry about that...", "The server is currently in development. ", 4) end)
local theme_names = {}
for name in pairs(themes) do
	table.insert(theme_names, name)
end
theme_picker = SovietUI.create_item_picker(
...
if get_debug_setting("enabled") then
	sfunction(function()
		...
		SovietUI.create_click_button("safe_function_err_test1", debugname, function() sfunction(nil) end)
		SovietUI.create_click_button("safe_function_err_test2", debugname, function() for i = 1, 100 do sfunction(function() local house = nil house:Destroy() end) end end)
		SovietUI.create_text_editor("translator_text_target", __translatortxt, debugname, function(txt)
			__translatortxt = tostring(txt) and txt
		end)
		SovietUI.create_text_editor("translator_lang_target", __translatorlangtarget, debugname, function(txt)
			__translatorlangtarget = tostring(txt) and txt
		end)
		SovietUI.create_click_button("translator_translate_process", debugname, function()
			...
			sfunction(function()
				...
				if __translator_translated then	
					SovietUI.display_message("debug.translator.result", tostring("D:translator.result="..__translator_translated))
				else
					SovietUI.display_message("debug:translator.result", "err:nil")
				end
			end)
		end)
		for i, v in ipairs(debugmode) do
			SovietUI.create_click_button(tostring(v..","..i), debugname, function(a)
